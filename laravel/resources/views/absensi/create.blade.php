        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('absensi.store') }}" method="POST" id="absensiForm">
                    ...
                    {{-- Bibit, Lokasi, Kandang --}}
                    <div class="row mb-4">
                        <div class="col-md-4">
                            <label class="form-label">Bibit *</label>
                            <select name="bibit_id" id="bibit_id" class="tom-select form-select @error('bibit_id') is-invalid @enderror" required>
                                <option value="">Pilih Bibit</option>
                                @foreach($bibits as $bibit)
                                <option value="{{ $bibit->id }}" 
                                        data-lokasi-id="{{ $bibit->lokasi_id }}" 
                                        data-kandang-id="{{ $bibit->kandang_id }}"
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="lokasi_id" id="lokasi_id" class="tom-select form-select @error('lokasi_id') is-invalid @enderror" disabled>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="kandang_id" id="kandang_id" class="tom-select form-select @error('kandang_id') is-invalid @enderror" disabled>
                                <option value="">Pilih Bibit terlebih dahulu</option>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered" id="karyawanTable">
                                ...
                                <tbody id="karyawanTableBody">
                                    {{-- Row pertama akan ditambahkan via JavaScript --}}
                                </tbody>
                            </table>
                        </div>
                        @error('karyawan.*')
...
<script>
    $(document).ready(function() {
        ...
        @if(old('karyawan'))
            ...
            @foreach(old('karyawan') as $index => $karyawanId)
                @php
                ...
                @endphp
                addKaryawanRow('{{ $karyawanId }}', '{{ $jabatanId }}', '{{ $jabatanNama }}', '{{ $tipeAbsen }}');
            @endforeach
        @else
        ...
        @if(old('bibit_id'))
            setTimeout(function() {
                const oldBibit = document.querySelector('#bibit_id option[value="{{ old('bibit_id') }}"]');
                if (oldBibit) {
                    bibitSelect.setValue('{{ old('bibit_id') }}');
